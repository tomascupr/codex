...
describe('Token streaming performance', () => {
  ...
  it('processes tokens with minimal delay', async () => {
    ...
    const intervals = tokenTimestamps.slice(1).map((t, i) => t - (tokenTimestamps[i] || 0));
    const avgDelay = intervals.length > 0 ? intervals.reduce((sum, i) => sum + i, 0) / intervals.length : 0;
